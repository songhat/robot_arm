...
def map_keyboard():
    ...
    bindings = {
        'q': (set_ee_target, ['position', np.asarray([pos_increment, 0, 0])], "x increase"),
        'a': (set_ee_target, ['position', np.asarray([-pos_increment, 0, 0])], "x decrease"),
        'w': (set_ee_target, ['position', np.asarray([0, pos_increment, 0])], "y increase"),
        's': (set_ee_target, ['position', np.asarray([0, -pos_increment, 0])], "y decrease"),
        'e': (set_ee_target, ['position', np.asarray([0, 0, pos_increment])], "z increase"),
        'd': (set_ee_target, ['position', np.asarray([0, 0, -pos_increment])], "z decrease"),
        'x': (reset_robot, [None], "reset to neutral pose")
     }
    if has_gripper:
        bindings.update({
        'r': (set_g, "close", "close gripper"),
        'f': (set_g, "open", "open gripper"),
        'c': (set_g, "calibrate", "calibrate gripper")
        })
    done = False
